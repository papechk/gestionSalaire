...
<div class="max-w-2xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Modifier employé</h2>

    <form action="{{ route('employers.update', $employer) }}" method="POST" class="bg-white rounded-lg shadow p-6 space-y-4">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="prenom" class="block text-sm font-medium text-gray-700 mb-1">Prénom</label>
                <input type="text" name="prenom" id="prenom" value="{{ old('prenom', $employer->prenom) }}"
            ...
            <div>
                ...
                <input type="text" name="nom" id="nom" value="{{ old('nom', $employer->nom) }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="contact" id="contact" value="{{ old('contact', $employer->contact) }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="departement_id" id="departement_id"
                    ...
                    @foreach($departements as $dept)
                        <option value="{{ $dept->id }}" {{ old('departement_id', $employer->departement_id) == $dept->id ? 'selected' : '' }}>{{ $dept->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="montant_journalier" id="montant_journalier" value="{{ old('montant_journalier', $employer->montant_journalier) }}" min="0"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500" required>
